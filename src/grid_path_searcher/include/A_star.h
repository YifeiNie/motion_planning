...
private: 
    ...
    float max_yz_idx = max_y_idx * max_z_idx;           // 用于三维数组的一维映射时，减少重复计算
...
public:
    ...
    inline Eigen::Vector31 coord2idx(int x, int y, int z);
    inline Eigen::Vector3d idx2coord(Eigen::Vector3i tem_idx);
    void set_obstacle(GridNode* GridNodePtr);
    void A_star_expand_neighbors(Eigen::vector3i idx);
